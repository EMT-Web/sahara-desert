...
async function importAgadirTours() {
  console.log('🚀 Starting Agadir tours import...\n')

  try {
    const client = getSanityClient()

    console.log('📦 Importing tours from Agadir...\n')
    const transformedTours = transformTours(toursFromAgadir)
    await createDocuments(client, 'tour', transformedTours, 500)
    console.log(`✅ Imported ${transformedTours.length} tours from Agadir\n`)

    console.log('✅ Agadir tours import complete!')
    console.log(`\n📊 Summary: ${transformedTours.length} tours imported`)
  } catch (error) {
    console.error('❌ Error importing Agadir tours:', error.message)
